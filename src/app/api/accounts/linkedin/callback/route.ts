...
import { exchangeCodeForToken, fetchMemberId } from "@/lib/connectors/linkedin";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const memberId = await fetchMemberId(token.access_token);

    await prisma.socialConnection.upsert({
      where: { userId_platformId: { userId, platformId: PlatformId.LINKEDIN } },
      update: {
        connected: true,
